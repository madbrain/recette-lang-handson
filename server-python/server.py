...
class RecetteLanguageServer(LanguageServer):
    ...
    def complete(self, doc: TextDocument, position: types.Position):
        ...
        if mode == IN_VERB:
            return (list(map(lambda verb: types.CompletionItem(kind=types.CompletionItemKind.Function, label=verb), VERBS.keys()))
                    + list(map(lambda verb: types.CompletionItem(kind=types.CompletionItemKind.Operator, label=verb), ADVERBS)))
...
def do_prepare_rename(ls: RecetteLanguageServer, params: types.PrepareRenameParams) -> types.Range | None:
    doc = ls.workspace.get_text_document(params.text_document.uri)
    return ls.prepare_rename(doc, params.position)

@server.feature(types.TEXT_DOCUMENT_RENAME)
def do_rename(ls: RecetteLanguageServer, params: types.RenameParams) -> types.WorkspaceEdit | None:
    doc = ls.workspace.get_text_document(params.text_document.uri)
